...
def run_optical_flow(vid_item, dev_id=0):
    ...
    cmd = '{}/build/extract_gpu -f={} -x={} -y={} -i={} -b=20 -t=1 -d={} -s=1 -o=zip'.format(CURRENT_DIR, vid_path, flow_x_path, flow_y_path, image_path, dev_id)
...
def run_warp_optical_flow(vid_item, dev_id=0):
    ...
    flow_x_path = '{}/flow_x'.format(out_full_path)
    flow_y_path = '{}/flow_y'.format(out_full_path)
    cmd = '{}/build/extract_warp_gpu -f {} -x {} -y {} -b 20 -t 1 -d {} -s 1 -o zip'.format(CURRENT_DIR, vid_path, flow_x_path, flow_y_path, dev_id)
    os.system(cmd)
...
def run_optical_flow_CPU(vid_item, dev_id=0):
    ...
    try: 
        ...
        pass
    cmd = '{}/build/extract_cpu -f={} -x={} -y={} -i={} -b=20 -t=1 -d={} -s=1 -o=zip'.format(CURRENT_DIR, vid_path, flow_x_path, flow_y_path, image_path, dev_id)
    print '>>> cmd: {}'.format(cmd)
    os.system(cmd)
    print '{} {} done'.format(vid_id, vid_name)
    return True
...
def run_warp_optical_flow_CPU(vid_item, dev_id=0):
    vid_path = vid_item[0]
    vid_id = vid_item[1]
    vid_name = vid_path.split('/')[-1].split('.')[0]
    out_full_path = os.path.join(out_path, vid_name)
    ...
    flow_x_path = '{}/flow_x'.format(out_full_path)
    flow_y_path = '{}/flow_y'.format(out_full_path)
    cmd = '{}/build/extract_cpu -f {} -x {} -y {} -b 20 -t 1 -d {} -s 1 -o zip'.format(CURRENT_DIR, vid_path, flow_x_path, flow_y_path, dev_id)
    os.system(cmd)
    print 'warp on {} {} done'.format(vid_id, vid_name)
    return True
